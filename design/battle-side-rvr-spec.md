...
- Heroes reuse the **proven XP curve** (level = √(xp/50), daily XP cap pattern from `leader-xp.ts`, **L3 one-time specialization** — the existing `scholar | marshal | steward` paths, war-flavored effects).
